...
from .models import Dimension, Portrait, Section, Tag
...
class LoveMetrics:
    ...
    def confidence(self) -> float:
        ...
        base = 0.2 + 0.7 * min(1.0, self.inputs.msg_sent / 40.0)
        signals = (
...
def compute_metrics(
    ...
    *,
    ...
    raw_nostalgia = (
        ...
        + inputs.partner_count * w.partner
    )
...
def headline_of(metrics: LoveMetrics) -> str:
    return f"{metrics.archetype.label}：{metrics.archetype.tagline}"
...
def fallback_portrait(metrics: LoveMetrics, *, target_name: str = "") -> Portrait:
    """LLM 不可用时的纯公式版画像。"""
    polarity = _ARCHETYPE_POLARITY.get(metrics.archetype.key, "neutral")
    judgment = metrics.archetype.reason
    ...
    sections = [
        ...
        Section(
            "数据说话",
            f"{who} 今天的恋爱成分 {metrics.total} 分（{total_label(metrics.total)}）。"
            f"纯爱值 {metrics.simp}、存在感 {metrics.vibe}、"
            f"白月光 {metrics.nostalgia}、败犬值 {metrics.ick}。",
        ),
        _breakdown_section(metrics),
    ]
    return Portrait(
        kind="love",
        headline=headline_of(metrics),
...
def merge_portrait(
    ...
    *,
    ...
    """
    base = fallback_portrait(metrics, target_name=target_name)
    if llm is None:
        return base
...
def metrics_prompt_block(metrics: LoveMetrics, *, target_name: str = "") -> str:
    ...
    lines = [
        f"{who} 今日互动指标（已按公式算好，请勿改动数字）：",
        f"- 纯爱值（主动投入）：{metrics.simp}",
        f"- 存在感（被回应）：{metrics.vibe}",
        f"- 白月光（话题引力）：{metrics.nostalgia}",
        f"- 败犬值（翻车下头）：{metrics.ick}",
        f"- 恋爱成分（综合）：{metrics.total}（{total_label(metrics.total)}）",
