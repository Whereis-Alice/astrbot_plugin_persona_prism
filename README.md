...
| 键 | 默认 | 说明 |
| --- | --- | --- |
| `use_astrbot_persona` | 关 | 把当前会话的 AstrBot 人格摘要接进提示词，只影响“用什么口吻写”，不影响维度分与证供 |
...
- 914 个单元测试覆盖清洗、抽样、提示词、解析、存储、卡片、Markdown 渲染、采集诊断、翻页自适应与自检、JSON 抢救、恋爱诊断公式与窗口、对话现场切片、人格联动、呈堂证供还原、头衔清洗、兼容层、WebUI 契约。
...
```bash
python -m pytest tests -q      # 914 passed
python -m ruff check .
```
...
v1.3.0 在它身上学到三件事：聊天现场里把每个人的真头像都拉出来、
模型输出的 JSON 碎了也要尽量抢救而不是直接放弃、以及翻群历史时按时间戳而不是按位置找“本页最旧的一条”。
