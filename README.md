...
下面 8 张都是真实渲染产物（示例数据，两倍分辨率），没有后期处理。
...
| | |
| --- | --- |
| 🎴 **5 套卡片主题 + 自动挡** | 极光玻璃 / 水墨宣纸 / 赛博霓虹 / 杂志排版 / 机密档案，每群可独立设置；也能交给自动挡按画像气质现挑 |
...
| 🤝 **协议端兼容** | go-cqhttp / NapCat / Lagrange / **LLBot（幸运莉莉娅）** 的历史回溯都能正常翻页，翻不动时用 `棱镜诊断` 一条指令查明原因 |
...
5. 想换卡片风格：`棱镜主题` 看列表，`棱镜主题 neon` 切换，`棱镜主题 auto` 让插件自己挑（管理员）。
...
除了这五套固定主题，还有一个档位 `auto`（**自动挡**）：不锁死风格，每张画像现挑一套。
...
1. **先看画像内容**。每套主题都挂着一组意象词——熬夜、玩梗、抽象算霓虹；沉稳、克制、留白算水墨；
   逻辑、干货、严谨算杂志；潜水、疏离、边界感算档案；温暖、共情、治愈算极光。
...
1. **请求参数叫 `message_seq`，但它认的编号不一定是 `message_seq`。** 一部分实现认的是消息的
   `message_id`，而这两者在很多协议端上是**两套完全不相干的编号**。
2. **返回的那一页数组，顺序也不固定。** 有的实现是「最旧 → 最新」，有的是「最新 → 最旧」，
   于是「这一页里最旧的那一条」到底在开头还是末尾也不一定。

两者组合出**四种**翻页方式，而**传错不会报错** —— 协议端会一声不响地原地返回同一批最新消息。
...
2. 一旦发现原地打转（或协议端压根没返回当前字段），就**依次换其余方式重试，四种全试一遍**；
   哪种有效就记进数据库，之后这个群直接沿用，不再反复试错。
3. 四种都翻不动，只记「协议端不支持翻页」并在采集诊断里说明，**不会**把这个群标成
...
**配置**：把 44 项常用配置按 9 组铺开，带说明文字与取值范围；
...
全部配置都能在 AstrBot 原生配置页改，其中 44 项也能在插件 WebUI 里改。下面按组说明。
...
不响地反复返回同一批最新消息。这个坑有两个自由度，v1.1.2/v1.1.3 都只修了一半，到 v1.1.4 才补齐：

- 协议端认哪个编号（`message_seq` 还是 `message_id`）—— v1.1.3 修；
- 返回的那一页数组是最旧在前还是最新在前，也就是「本页最旧的一条」到底在 `messages[0]` 还是
  `messages[-1]` —— v1.1.4 修。

**先升级到 v1.1.4，然后让管理员在群里发一次 `棱镜诊断`**：它会把四种翻页方式各打一次真实请求，
...
- 5 套手写卡片主题（外加按画像气质现挑的自动挡）+ 四层渲染兜底，渲染失败也一定有结果；
...
- 475 个单元测试覆盖清洗、抽样、提示词、解析、存储、卡片、Markdown 渲染、采集诊断、翻页自适应与自检、兼容层、WebUI 契约。
