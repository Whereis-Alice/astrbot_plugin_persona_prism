...
| `棱镜姻缘` | 在群里给 TA 找最合得来的搭子：缘分榜 + 匹配度 + 破冰指南 | 所有人 |
...
- 1017 个单元测试覆盖清洗、抽样、提示词、解析、存储、卡片、Markdown 渲染、采集诊断、翻页自适应与自检、JSON 抢救、恋爱诊断公式与窗口、对话现场切片、模型选段与三级兜底、人格联动与人格铭牌、呈堂证供还原、头衔清洗、兼容层、WebUI 契约。
...
```bash
python -m pytest tests -q      # 1017 passed
python -m ruff check .
```
