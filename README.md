...
本群语料状态：
  ...
  历史回溯：仍可继续回溯
  上次回溯：14 分钟前
  回溯断点：message_seq=88123
```

「上次回溯」有时间就说明轮询成功过；「回溯断点」是下次继续往前挖的位置。
...
历史回溯靠 OneBot 的 `get_group_msg_history`，而这个接口的**翻页游标**在各家实现里
含义不太一样，是个经典坑：

| 协议端 | 情况 |
| --- | --- |
| go-cqhttp | 返回 `message_seq`，正常 |
| NapCat | `message_id` 可能是**负数** |
| Lagrange | 同上 |
| **LLBot（幸运莉莉娅）** | `message_id` 与 `message_seq` 是**两套完全独立的编号** |

本插件取游标的顺序是 `message_seq` → `real_seq` → `message_id`，
并且用整数解析而不是 `str.isdigit()`，所以负数 ID 和双编号都能正确翻页。
上游用的是 `message_id` + `isdigit()`，在 NapCat 的负数 ID 和 LLBot 的双编号下
会**静默停在第一页** —— 表现就是「怎么画都说语料不足」。
...
**配置**：把 42 项常用配置按 9 组铺开，带说明文字与取值范围；
...
全部配置都能在 AstrBot 原生配置页改，其中 42 项也能在插件 WebUI 里改。下面按组说明。
...
「上次回溯」和「回溯断点」也会留痕。详见[怎么确认回溯真的跑了](#怎么确认回溯真的跑了)。
...
| 4 | 历史翻页游标取 `message_id` 且用 `isdigit()` 判断，在 NapCat 负数 ID 与 LLBot 双编号下静默停在第一页 | 游标优先 `message_seq` → `real_seq` → `message_id`，整数解析，负数照样能翻 |
...
- 444 个单元测试覆盖清洗、抽样、提示词、解析、存储、卡片、Markdown 渲染、采集诊断、兼容层、WebUI 契约。
