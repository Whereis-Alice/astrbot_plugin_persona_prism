...
| 🗣 **看的是对话不是独白** | 模型不只看 TA 自己的发言，还会看到前后几句对话，知道 TA 在回应谁、别人又怎么接，“嗯”“对啊”不会再被读成自言自语 |
| 🧠 **结构化分析** | 提示词强制 JSON 输出：标签、维度评分、呈堂证供、置信度，卡片不再是一坨纯文本 |
| 🏅 **专属头衔** | 每张卡片在昵称下方压一枚称号徽章（如「纯爱战神（反讽）」），恋爱诊断按四维实分推导，画像/棱镜优先用模型给的、模型不给就按玩法兜底 |
...
- **喂给模型的是对话，不是独白。** 抽样到 TA 的发言后，会顺手把它前后各几句的群聊一起切出来，
  拼成一段带时间戳的「对话现场」。回应型发言才不会被误读成自言自语，关系类结论也有现场可引。
...
每张卡片在昵称下方都会压一枚称号徽章，比如「纯爱战神（反讽）」「深夜哲学家」「话密度满格」。
徽章样式跟着主题走：极光是 `✦ 称号`，水墨是朱印「印封号」，霓虹是 `▰ TITLE`，
杂志是「本期标题」，档案袋是「代号」，樱粉是「专属头衔」。
...
| 键 | 默认 | 说明 |
| --- | --- | --- |
| `context_enabled` | 开 | 开启后提示词里多一段「对话现场」，模型能看到 TA 在回应谁、别人又怎么接 |
| `context_span` | 3 | 每个现场前后各取几句上下文，0–8；0 等于只给 TA 自己的话 |
| `max_scenes` | 12 | 最多选几个现场，1–40 |
| `max_lines` | 80 | 对话现场最多占多少行，10–300；越大越费 token |
| `social_signals` | 开 | 额外给模型一组互动统计（被回应率、主动开口比例、常与谁一来一回） |
...
按时间均匀选几个 TA 参与的片段，每个片段把前后几句一并给出，并标清 `[TA]` / `[其他人]` / `[机器人]`，
跟帖、被 @ 、被回复这些位置优先入选。另外还会附一组互动统计（说完话多少次被接、多少句是主动开口、
常与谁一来一回）。“对谁热情”“是不是自言自语”这类结论从此有现场可依，而不是靠猜。
...
| 18 | 只把目标本人的发言排成一列喂给模型，“嗯”“那行吧”这种回应容易被读成自言自语 | 提示词里多一段「对话现场」：按时间切出 TA 发言前后的群聊，区分 TA / 其他人 / 机器人，并附上被回应率等互动信号，关系类结论必须有现场支持 |
...
| 17 | 机器人自己的黑名单靠 hasattr(event, "self_id") 判断（AstrBot 事件上并没有这个属性），实际从未生效 | 机器人身份走 event.get_self_id()，它的发言只在对话现场里作背景出现，不给任何人加分 |
...
- 对话现场：模型看到的不只是 TA 的独白，还有 TA 发言前后的群聊上下文与被回应率，关系类结论不得凭空；
- 可选接入 AstrBot 自带人格（会话人格 / 指定人格 / 全局默认），并能把本插件的请求交给其他插件的 `on_llm_request` 钩子加料（如世界树设定集）；
- 每张卡片都有一枚专属头衔徽章（六套主题各有自己的徽章语汇），恋爱诊断的头衔完全由四维实分推导；
- 829 个单元测试覆盖清洗、抽样、提示词、解析、存储、卡片、Markdown 渲染、采集诊断、翻页自适应与自检、恋爱诊断公式与窗口、对话现场切片、人格联动、呈堂证供还原、头衔清洗、兼容层、WebUI 契约。
