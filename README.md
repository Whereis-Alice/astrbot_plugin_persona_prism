...
AstrBot 插件 · 本地语料库 + 结构化 LLM 分析 + 10 套卡片主题 + 独立 WebUI 管理台
...
下面 13 张都是真实渲染产物（示例数据，两倍分辨率），没有后期处理。

### 棱镜系列 · 结构化信息卡

一张卡里有：专属头衔与 16 型人格铭牌、人格标语、极性标签、维度评分 + 雷达图、分节点评、
呈堂证供（仿聊天截图的现场气泡，带头像与时刻）、相处建议、置信度与执笔署名。
...
| 牛皮纸档案袋 + 打字机字体 | 粉调渐变 + 恋爱游戏味小节符号 |

| 画像系列 · 长文卡（`dossier` 主题） | |
| :---: | :---: |
| <img src="assets/themes/markdown_dossier.jpg" alt="画像系列长文卡片" width="420"> | 上游同款长文报告，走本插件的排版与主题 |

> 上面这 6 套是**画像主题池**，对「棱镜系列」与「画像系列」生效。全局设一个默认值，每个群还能用 `棱镜主题 neon` 单独覆盖；
> 懒得选就设 `棱镜主题 auto`，让插件按每张画像的气质现挑一套。

### 恋爱诊断 · 专属主题

恋爱诊断卡的气质和人格分析卡不一样，所以它有**自己的四套主题池**，不会被「机密档案」这类不搭的皮抽到：

| 恋色樱粉 `sakura` | 月下夜话 `moonlit` |
| :---: | :---: |
| <img src="assets/themes/love_sakura.jpg" alt="恋爱诊断卡（恋色樱粉）" width="420"> | <img src="assets/themes/love_moonlit.jpg" alt="恋爱诊断卡（月下夜话）" width="420"> |
| 粉紫糖果色，默认主题 | 深靛夜色 + 月华金，适合暗恋与深夜拉扯 |

| 黄昏电车 `dusk` | 莓果苏打 `berry` |
| :---: | :---: |
| <img src="assets/themes/love_dusk.jpg" alt="恋爱诊断卡（黄昏电车）" width="420"> | <img src="assets/themes/love_berry.jpg" alt="恋爱诊断卡（莓果苏打）" width="420"> |
| 橘紫残阳 + 胶片颗粒，适合久处与回忆 | 苏打蓝撞莓果粉，适合直球与热恋 |

> 恋爱卡的主题用 `love.theme` 单独配，默认恋色樱粉；填 `auto` 就每次从这四套里按诊断结果的气质现挑。
> 两个主题池互不干扰，证供区抬头、头衔铭牌的花饰、面板标题也会跟着主题一起换。
...
| | |
| --- | --- |
| 🎴 **10 套卡片主题 + 自动挡** | 画像六套（极光玻璃 / 水墨宣纸 / 赛博霓虹 / 杂志排版 / 机密档案 / 恋色樱粉）+ 恋爱卡专属四套（恋色樱粉 / 月下夜话 / 黄昏电车 / 莓果苏打），每群可独立设置；也能交给自动挡按卡片气质现挑 |
...
| 🖥 **独立 WebUI** | 配置面板 + 画像记录管理，按群 / 用户分类且显示群名与昵称，顶部 Tab 布局，4 套界面主题 + 跟随系统 |
...
| 🧩 **16 型人格铭牌** | 头衔旁边再挂一格类型徽章，叫法三选一：官方译名 `ENTP · 辩论家`、玩梗版 `杠精之王`、二次元版 `中二病主角`，不喜欢可以关 |
| 🎭 **跟你 Bot 一个调** | 默认借用 AstrBot 当前会话的人格来写卡片文案，抬头署上「执笔 · 某某」；同一个人在不同 Bot 手里读起来是两份不同的报告，结论口径不变 |
...
三套共用同一个语料库、同一套额度与隐私策略、同一套卡片引擎，区别在提示词契约、计分方式、排版模板与主题池。
...
| 指令 | 也可以这样发 | 作用 | 权限 |
| --- | --- | --- | --- |
| `棱镜画像` | `人格画像` | 中立视角的人格画像：标签、维度雷达、呈堂证供、建议 | 所有人 |
| `棱镜赞赏` | `群友赞赏` | 只挑优点夸，适合发群公告 / 生日祝福 | 所有人 |
| `棱镜锐评` | `群友锐评` | 犀利吐槽版，有底线（不涉外貌、身份、隐私攻击） | 所有人 |
| `棱镜姻缘` | `群友姻缘` | 在群里给 TA 找最合得来的搭子：缘分榜 + 匹配度 + 破冰指南 | 所有人 |
| `棱镜克隆` | `人格克隆` | 把某人的说话风格提炼成一份人格提示词 | 管理员 |

> 第二列是卡片标题上印的名字。群友看到卡片写着「群友姻缘」，照着发一遍就能出卡，
> 不用回头翻帮助。两种写法完全等价，参数（`@某人` / QQ 号）也一样能跟。

> **棱镜画像和棱镜姻缘有什么不一样？** 画像看的是「这个人是什么样」，只讲 TA 自己；
> 姻缘看的是「这个人和谁最搭」，卡片顶部会给出一张缘分榜 —— 谁接住过 TA 的话、TA 主动找过谁、
> 两人有没有常待在同一场，都是本地数出来的，模型只负责把名单写成人话，不会凭空给你指个对象。
...
- 翻页方式会**自适应**：这个接口有两处各家实现不统一的地方（认哪个编号、返回的一页是最旧在前
  还是最新在前），插件会把六种组合逐个试出来并记住。详见[协议端兼容性](#协议端兼容性)。
...
如果诊断里出现「协议端的翻页没有继续前进」，说明六种翻页方式都试过了但协议端就是不给翻页。
先让管理员在群里发一次 `棱镜诊断`确认（它会把每种方式的实测结果摊开来给你看）；如果确实六种
...
| `love.theme` | `sakura` | 恋爱卡专属主题，只能填 `sakura` / `moonlit` / `dusk` / `berry`，或 `auto` 走自动挡；与 `棱镜主题` 互不影响 |
...
十套主题都是手写 HTML + CSS，自包含、零外链、零脚本。它们分成**两个互不干扰的池子**：

- **画像主题池**（六套）：给「棱镜系列」「画像系列」与「棱镜帮助」用，用 `棱镜主题` 切。
- **恋爱主题池**（四套）：只给「恋爱诊断」用，用配置项 `love.theme` 切。

两边都能全局设默认，画像池还能每群单独设。
...
| `sakura` | 恋色樱粉 | Y2K 恋爱游戏风，粉紫渐变 + 圆角糖果色（也在恋爱池里）|

恋爱池四套：

| 主题 | 名字 | 长相 |
| --- | --- | --- |
| `sakura` | 恋色樱粉 | 粉紫糖果色，恋爱卡默认 |
| `moonlit` | 月下夜话 | 深靛夜色 + 月华金，适合暗恋与深夜拉扯 |
| `dusk` | 黄昏电车 | 橘紫残阳 + 胶片颗粒，适合久处与回忆 |
| `berry` | 莓果苏打 | 苏打蓝撞莓果粉，适合直球与热恋 |

两个池子各自还有一个 `auto` 档位（**自动挡**）：不锁死风格，每张卡从自己那个池子里现挑一套。
...
恋爱卡的自动挡同理，只是换了一套看盘口径：看四维实分的形态——直球、热恋、互动密切偏苏打；
暗恋、深夜、单向拉扯偏月下；白月光、久处、回忆味偏黄昏；心动、纠结、纯爱偏樱粉。
语料不多、结论没把握时偏月下夜话（而不是画像池那个机密档案）。同样会避开本群最近用过的两套恋爱主题。
...
十套主题各有自己的折角配色与纹样：极光是冰蓝 `✦`，水墨是朱红 `❖`，霓虹是直角切边 `◤◥`，
...
模型那一层也有兜底：大模型偶尔会把 JSON 写破（输出被长度限制砍断、把逗号冒号引号敲成全角）。
以前这一次画像就白花了，现在会按“标点修正 → 残片抢救”逐步修补，能抢回多少就用多少，
宁可少几个字段也不让你白等一场；真的修过会写一行日志，群里不打扰你。
...
2. **返回的那一页数组，顺序也不固定。** 有的实现是「最旧 → 最新」，有的是「最新 → 最旧」，
   还有的干脆不排序。于是「这一页里最旧的那一条」到底在开头、末尾还是夹在中间，光看位置是猜不出来的。
3. **参数名本身也不统一。** 老一批协议端读 `message_seq`，SnowLuma 这类只读 `message_id`、
   压根不看 `message_seq`；倒序开关也有 `reverseOrder` 和 `reverse_order` 两种拼写。

前两项组合出**六种**翻页方式（2 个编号字段 × 3 种取值：本页第一条 /
最后一条 / 按时间戳算出最旧的一条），而**传错不会报错** —— 协议端会一声不响地
原地返回同一批最新消息。
...
| 个别实现 | 返回的一页**根本不按时间排序**，只能逐条比时间戳才知道哪条最旧 |
...
3. 一旦发现原地打转（或协议端压根没返回当前字段、或直接回了空页），就**依次换其余方式重试，
   六种全试一遍**；哪种有效就记进数据库，之后这个群直接沿用，不再反复试错。
   其中最后两种不看位置，而是把本页每条的时间戳比一遍，找出真正最旧的那条当锚点 —— 专治「返回不排序」的实现。
4. 六种都翻不动，只记「协议端不支持翻页」并在采集诊断里说明，**不会**把这个群标成
...
如果某个群怎么都攒不到语料，管理员在**群里**发 `棱镜诊断`。它会用 `count=20` 打七次真实请求
（首页 + 六种方式各一次），**全程不写入语料库**，然后把实测结果摊开来：
...
往前翻一页的六种方式：
  ...
  · [跳过] message_seq（取本页时间最早的一条）：前面已经试过同一个游标值
  · [跳过] message_id（取本页时间最早的一条）：前面已经试过同一个游标值
...
从 `auto` 锁定为 `seq_first` / `id_first` / `seq_last` / `id_last` / `seq_oldest` /
`id_oldest`，省掉试探请求。
...
界面分三层：最上面是标题栏（切主题、切信息密度、刷新），紧接着一排横向 Tab 管四个板块，
每个板块开头有一张横幅写清这一页能干什么、顺手报出画像数 / 群聊数 / 群友数 / 语料量；
页面底部常驻一条状态条，滚到哪儿都看得到当前规模、出图链路与数据时间。

| | |
| --- | --- |
| <img src="assets/webui/webui_nocturne.jpg" alt="概览页（夜曜主题）" width="420"> | <img src="assets/webui/webui_daylight.jpg" alt="画像记录页（晴昼主题）" width="420"> |
| **概览 · 夜曜** 一屏看完运行状态 | **画像记录 · 晴昼** 左侧群聊目录 + 右侧记录流 |
| <img src="assets/webui/webui_ink.jpg" alt="运行设置页（墨宣主题）" width="420"> | <img src="assets/webui/webui_neon.jpg" alt="提示词页（霓境主题）" width="420"> |
| **运行设置 · 墨宣** 分组表单，改完即时生效 | **提示词 · 霓境** 内置模板可改，也能自己加玩法 |

**概览**：画像总数 / 今日新增、覆盖群聊与人数、语料规模、近 7 天成功率与平均耗时、
当前渲染链路与上次实际用到的后端、各套玩法被用了多少次、开关状态一览。

**画像记录**：按**群**再按**用户**分层的树形目录，群名称与群友昵称都会显示（不是只给你一串 ID）。
目录能一键**收起**，收起后记录列表铺满整页，专心翻记录时不占地方；下次进来还是你上次的状态。
每条记录带群友头像、头衔铭牌、玩法标签与取材规模，支持关键词搜索、按玩法筛选
（下拉项直接读后端提示词表，自定义玩法与兼容指令的记录都能筛）、分页、点开看完整正文与
当时那张卡片、单条删除、按群 / 按人批量清空。
...
标题栏右边还能切「舒适 / 紧凑」两档信息密度：紧凑档把留白与字号整体收一档，
一屏能多看好几条记录。主题与密度都记在浏览器本地（`localStorage`），刷新不丢。
页面零外链、零 CDN、零内联脚本，
...
| `cursor_field` | `auto` | 历史翻页方式。`auto` 会把六种组合都试一遍并记住有效的那种；也可锁定成 `seq_first` / `id_first` / `seq_last` / `id_last` / `seq_oldest` / `id_oldest`（发 `棱镜诊断` 可以查出本群该选哪个） |
...
| `llm_chain` | 开 | 让模型先从带编号的群聊里挑出「哪几句构成一场对话」，再按编号回库取原文；关掉则退回按时间与条数切窗口 |
| `llm_chain_scenes` | 5 | 上面这一步最多挑几场对话，1–12；挑得多现场更全，也更费一点 token |

> 关掉 `context_enabled` 会回到 v1.2.3 之前的“只看独白”口径，token 略省，但关系类结论（对谁热情、
> 是不是在自言自语）准确率会明显下降。

> `llm_chain` 多花一次很便宜的小请求（只让模型回一串编号，不产出文字），换来的是卡片里的聊天现场
> 能连成完整一段对话，而不是散落几句。这一步失败会自动退回本地切窗口，不影响出卡。

### `profile` 人格标签

| 键 | 默认 | 说明 |
| --- | --- | --- |
| `type_tag` | `mbti` | 卡片头衔右侧那枚 16 型人格铭牌用哪套叫法，四档见下 |

四档任选：

- `off` —— 不显示这枚铭牌。
- `mbti` —— 官方译名，例：`ENTP · 辩论家`。
- `sbti` —— 玩梗版，例：`ENTP · 杠精之王`。
- `acgti` —— 二次元版，例：`ENTP · 中二病主角`。

类型由模型在分析时一并给出（依据同一份语料），换档只换叫法、不会重算。
棱镜系列、画像系列、恋爱诊断三种卡片都会挂这枚铭牌。

### `persona` 人格联动（默认开）

装了这个插件的人基本都给自己的 Bot 配了人格，所以这组默认是**开**的：卡片里的叙述会用你 Bot
平时的口吻来写，卡片抬头也会署上一行「执笔 · 某某」。想要一份语气中立的报告，把它关掉即可。

| 键 | 默认 | 说明 |
| --- | --- | --- |
| `use_astrbot_persona` | 开 | 把当前会话的 AstrBot 人格摘要接进提示词，只影响“用什么口吻写”，不影响维度分与证供 |
| `persona_id` | 空 | 锁定一个人格名；留空则自动跟随本会话实际生效的人格（含 WebUI 会话管理里单独钉的那个），都没设才用全局默认 |
| `allow_llm_hooks` | 关 | 允许其他插件的 `on_llm_request` 钩子改写本插件的请求 |

几个要点：

- 人格只给“叙述风格”，不给“结论”。提示词里它被限定在 900 字以内、且明确标注为口吻参考，
  维度评分、呈堂证供、恋爱四维分这些依然走原有口径。
- 已经在会话里手动关掉人格（AstrBot 的“无人格”）的群，不会被拽回全局默认人格。
- 人格按 AstrBot 自己的真实优先级解析：**插件里指定的 `persona_id` > WebUI「会话管理」里给这个群
  单独钉的人格 > 当前对话的人格 > 全局默认人格**。之前只读后两层，给单个群强制指定的人格会被漏掉，v1.3.0 修了。
- 想确认到底套用了哪个人格，发一次 `棱镜诊断`，末尾会多一行「文案口吻：…」把结果告诉你；
  更详细的（人格名、来源那一层、字数、钩子有没有改动提示词）会写进 AstrBot 后台日志，搜「套用人格」即可。
...
连同它所在的那一个聊天回合一起渲染成聊天气泡（带每个人的真头像与昵称）。
气泡里的每个字、每个昵称都出自数据库，模型没有机会替别人说话；
...
**先升级到最新版，然后让管理员在群里发一次 `棱镜诊断`**：它会把六种翻页方式各打一次真实请求，
...
同时写进两个参数名，并且六种翻页方式里的 `message_id` 那几种会被自动试出来。若还是攒不到语料，
...
**Q：卡片标题写的是「群友姻缘」，为什么我发这四个字没反应？**
A：v1.5.2 起有反应了。棱镜系列 5 个玩法的卡片标题（`人格画像` `群友赞赏` `群友锐评`
`群友姻缘` `人格克隆`）现在都是对应指令的别名，和 `棱镜xx` 完全等价，参数写法也一样。
发 `棱镜帮助`，指令名旁边带虚线框的小字就是它的别名。
...
| 4 | 历史翻页游标用 `isdigit()` 判断，NapCat 的负数 `message_id` 会被直接丢掉，静默停在第一页 | 整数解析，负数照样能翻；六种翻页方式（编号字段 × 取本页哪一端，含按时间戳挑出最旧一条）全部自动试探并记住有效的那种，翻不动只报诊断而不误判成「历史见底」，另有 `棱镜诊断` 可实测 |
...
| 14 | 证据只是几行干巴巴的引用 | 「呈堂证供」把原话对回语料库，连同它所在的那一个聊天回合渲染成聊天气泡（每个人都是真头像与真昵称），并按场景（冷场破冰 / 接话现场 / 深夜时段…）配小标题 |
...
- 10 套手写卡片主题（画像 6 + 恋爱卡专属 4，各带一个按气质现挑的自动挡）+ 四层渲染兜底，渲染失败也一定有结果；
- 高清渲染（默认 2 倍，最高 3 倍）+ 图片格式/质量可配 + 自定义字体内嵌；
- 独立 WebUI：配置面板、可收纳的群聊目录 + 记录管理、提示词编辑、隐身名单、4 套界面主题与两档信息密度；
...
- 聊天现场里的每个人都是**真头像**（不只主角），机器人自己的气泡会标上角标，看起来就像一张真的聊天截图；
- 模型把 JSON 写破时能就地修补（全角标点、被砍断的输出都能抢救），不会把一次额度白白浪费；
- 可选接入 AstrBot 自带人格，按它自己的优先级解析（指定人格 > 会话管理里钉的人格 > 当前对话人格 > 全局默认），并能把本插件的请求交给其他插件的 `on_llm_request` 钩子加料（如世界树设定集）；
- 每张卡片都有一枚奖带铭牌式的专属头衔（十套主题各有自己的折角配色与纹样），恋爱诊断的头衔完全由四维实分推导；
- 棱镜系列的卡片标题同时是指令别名（`人格画像`＝`棱镜画像`…），照着卡片发就能用；
- 1069 个单元测试覆盖清洗、抽样、提示词、解析、存储、卡片、Markdown 渲染、采集诊断、翻页自适应与自检、JSON 抢救、恋爱诊断公式与窗口、对话现场切片、模型选段与三级兜底、人格联动与人格铭牌、呈堂证供还原、头衔清洗、兼容层、WebUI 契约。
...
```bash
python -m pytest tests -q      # 1069 passed
python -m ruff check .
```
...
同样感谢 **[SXP-Simon](https://github.com/SXP-Simon)** 的
**[astrbot_plugin_qq_group_daily_analysis](https://github.com/SXP-Simon/astrbot_plugin_qq_group_daily_analysis)**。
从它身上学到四件事：聊天现场里把每个人的真头像都拉出来、模型输出的 JSON 碎了也要尽量抢救而不是直接放弃、
翻群历史时按时间戳而不是按位置找“本页最旧的一条”，以及给分析结果配一枚 16 型人格标签（本插件的人格铭牌由此而来，
并扩成官方译名 / 玩梗版 / 二次元版三套叫法）。
