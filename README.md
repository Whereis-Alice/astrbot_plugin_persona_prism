...
三套玩法：结构化人格卡「棱镜系列」· 上游同款长文卡「画像系列」· 每日互动结算「恋爱成分」
...
| 牛皮纸档案袋 + 打字机字体 | 粉调渐变 + 恋爱游戏味小节符号，恋爱成分默认主题 |
...
v1.2.0 又并入了「恋爱成分」这类每日互动结算玩法（上游 `astrbot_plugin_love_formula`）。
...
| 🔀 **三套玩法并存** | 「棱镜系列」出结构化信息卡，「画像系列」出上游同款长文卡，「恋爱成分」出每日互动结算，互不干扰 |
| 💗 **恋爱成分日结** | 纯公式算出纯爱 / 好感 / 下头 / 情怀四维分与人设归类，可复现、不看模型脸色；判词才交给模型润色 |
...
| | 棱镜系列 | 画像系列（兼容上游） | 恋爱成分 |
| --- | --- | --- | --- |
| 指令 | `棱镜画像` `棱镜赞赏` `棱镜锐评` `棱镜姻缘` `棱镜克隆` | `画像` `正画像` `负画像` `找对象` `克隆人格` | `棱镜恋爱` `棱镜恋爱榜` |
| 看的是 | 长期语料里的说话风格 | 长期语料里的说话风格 | **今天一天**的互动行为 |
| 分数来自 | 模型判断 | 模型判断（不打分） | **公式计算**，可复现 |
| 模型输出 | 强制 JSON（标签 / 维度分 / 证据 / 置信度） | 自由长文（Markdown） | 只写判词，不碰分数（可关） |
| 卡片长相 | 结构化信息卡：雷达图 + 评分条 + 引用块 | 排版长文卡：标题、列表、表格、引用 | 五维雷达 + 成分拆解 + 昨日趋势 |
| 适合 | 想要「一眼看懂」的评分与对比 | 想要「读起来过瘾」的报告 | 每天来一发的日常小游戏 |
| 附带指令 | `棱镜档案`（重发卡片图） | `查看画像`（重发纯文本，方便复制）、`切换人格` / `恢复人格` | 兼容别名 `今日人设` |
...
| `compat.love_commands` | 兼容别名 `今日人设` 下线，可与 `astrbot_plugin_love_formula` 共存 |
...
5. 想玩每日结算：发 `棱镜恋爱` 看今天的恋爱成分，`棱镜恋爱榜` 看全群排名。
...
受 `love.enabled` 开关控制，默认开启。详见[恋爱成分是怎么算的](#恋爱成分是怎么算的)。

| 指令 | 作用 | 权限 |
| --- | --- | --- |
| `棱镜恋爱` | 结算今天的恋爱成分：四维分数、人设归类、成分拆解、昨日趋势 | 所有人 |
| `棱镜恋爱榜` | 本群今日恋爱成分排行，**不调模型**，秒回 | 所有人 |
| `今日人设` | `棱镜恋爱` 的兼容别名（上游同名指令），受 `compat.love_commands` 控制 | 所有人 |
...
「恋爱成分」是每天一结算的轻量玩法，看的不是你说了什么，而是**今天这一天里你怎么互动**。
...
```text
恋爱成分 = clamp( ((存在感 + 白月光) - (败犬值 + 纯爱值) + 200) / 4 , 0, 100 )
```

直觉上就是：**别人主动找你**加分，**你单方面用力**扣分，**翻车**扣得更狠。
纯爱值高、存在感低 = 独角戏；反过来 = 众人所向。四项原始分全为 0（当天几乎没数据）时综合分固定 50，不瞎猜。
...
| `love.min_messages` | 5 | 当天发言少于这个数就不出卡 |
...
| `love.leaderboard_size` | 10 | `棱镜恋爱榜` 列几名，3~30 |
| `love.notice_collect` | 开 | 是否监听戳一戳 / 表情回应 / 撤回通知 |
| `love.theme` | `sakura` | 恋爱卡专用主题，独立于 `棱镜主题`；可填 `auto` 走自动挡 |
| `compat.love_commands` | 开 | 兼容别名 `今日人设` 的开关 |

> 隐身（`棱镜隐身`）和保护名单对恋爱成分同样有效：既不出卡，也整体从排行榜里排除。
## 卡片主题与清晰度
...
| `sakura` | 恋色樱粉 | Y2K 恋爱游戏风，粉紫渐变 + 圆角糖果色，恋爱成分卡默认 |
...
**配置**：把 54 项常用配置按 10 组铺开（含「恋爱成分」组），带说明文字与取值范围；
两个敏感开关（同步机器人昵称 / 头像）在这里**只读展示**，只能在 AstrBot 原生配置页改，
避免误点一下就把机器人的全局资料给改了。

**提示词**：内置 11 套（棱镜 5 + 画像 5 + 恋爱成分 1）可开关、可改写；也能新增自定义玩法
...
| `love_commands` | 开 | 是否启用兼容别名 `今日人设`。要和 `astrbot_plugin_love_formula` 共存就关掉 |
## 画像准不准
...
或 `compat.love_commands`（今日人设）关掉即可。

**Q：恋爱成分和人格画像有什么区别？**
A：人格画像看的是**长期语料里的说话风格**，分数由模型判断；恋爱成分看的是**今天一天的互动行为**，
分数由公式算、可复现。前者适合偶尔来一发，后者适合每天签到。详见
[恋爱成分是怎么算的](#恋爱成分是怎么算的)。

**Q：协议端不下发戳一戳 / 表情回应通知，恋爱成分还能用吗？**
A：能。这三项会恒为 0，其余维度（发言、回复、艾特、图片、复读、刷屏、深夜）全部从语料库现算，
照常出卡。想彻底关掉这部分采集就把 `love.notice_collect` 关掉。

**Q：恋爱成分一直说「今天只说了 0 句」，但人格画像正常？**
...
「恋爱成分」玩法的创意与四维框架来自 `astrbot_plugin_love_formula`。并入时重写了公式与数据层：
...
- 恋爱成分玩法的分数**完全可复现**：公式在纯函数模块里，模型只负责润色文案，关掉模型也能出卡；
- 627 个单元测试覆盖清洗、抽样、提示词、解析、存储、卡片、Markdown 渲染、采集诊断、翻页自适应与自检、恋爱成分公式与持久层、兼容层、WebUI 契约。
...
```bash
python -m pytest tests -q      # 627 passed
python -m ruff check .
```
...
「把一天的群聊互动折算成恋爱成分、再归类成人设」这个点子，以及纯爱 / 好感 / 下头 / 情怀
这个四维框架和一批人设命名，都出自它。v1.2.0 的恋爱成分玩法是在这个框架上重写公式与数据层，
并保留了 `今日人设` 这个指令名做兼容。
