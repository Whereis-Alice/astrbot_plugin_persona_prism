...
兼容上游 `astrbot_plugin_portrayal` 的「画像」系列指令 · 全部产出用本插件的卡片渲染
...
下面 6 张都是真实渲染产物（示例数据，两倍分辨率），没有后期处理。

### 棱镜系列 · 结构化信息卡

一张卡里有：人格标语、极性标签、维度评分 + 雷达图、分节点评、原话证据、相处建议、置信度与模型署名。
...
| 机密档案 `dossier` | 画像系列 · 长文卡（`dossier` 主题） |
| :---: | :---: |
| <img src="assets/themes/theme_dossier.jpg" alt="机密档案主题卡片" width="420"> | <img src="assets/themes/markdown_dossier.jpg" alt="画像系列长文卡片" width="420"> |
| 牛皮纸档案袋 + 打字机字体 | 上游同款长文报告，走本插件的排版与主题 |

> 5 套主题对两种玩法都生效。全局设一个默认值，每个群还能用 `棱镜主题 neon` 单独覆盖。
...
| 🔀 **两套玩法并存** | 「棱镜系列」出结构化信息卡，「画像系列」出上游同款长文卡，互不干扰 |
...
| 🤝 **协议端兼容** | go-cqhttp / NapCat / Lagrange / **LLBot（幸运莉莉娅）** 的历史回溯都能正常翻页 |
...
| | 棱镜系列 | 画像系列（兼容上游） |
| --- | --- | --- |
| 指令 | `棱镜画像` `棱镜赞赏` `棱镜锐评` `棱镜姻缘` `棱镜克隆` | `画像` `正画像` `负画像` `找对象` `克隆人格` |
| 模型输出 | 强制 JSON（标签 / 维度分 / 证据 / 置信度） | 自由长文（Markdown） |
| 卡片长相 | 结构化信息卡：雷达图 + 评分条 + 引用块 | 排版长文卡：标题、列表、表格、引用 |
| 适合 | 想要「一眼看懂」的评分与对比 | 想要「读起来过瘾」的报告 |
| 附带指令 | `棱镜档案`（重发卡片图） | `查看画像`（重发纯文本，方便复制）、`切换人格` / `恢复人格` |

两套共用同一个语料库、同一套额度与隐私策略、同一批卡片主题，区别只在提示词契约与排版模板。

**和上游同时装会不会撞指令？** 会。`画像` 这类指令名和上游完全一样，两个插件同时启用时
AstrBot 会同时触发。所以要么只留一个插件，要么把本插件的 `compat.legacy_commands` 关掉
（关掉后「画像系列」全部下线，「棱镜系列」照常工作，此时可与上游共存）。
...
5. 想换卡片风格：`棱镜主题` 看列表，`棱镜主题 neon` 切换（管理员）。
6. 想翻旧账：`棱镜历史`，或者打开 WebUI 的「人格棱镜」页面慢慢看。
...
- 翻页游标会**自适应**：协议端认哪个字段（`message_seq` 还是 `message_id`）由插件自动试探，
  试出来的结果会记住。详见[协议端兼容性](#协议端兼容性)。
...
回溯是后台行为，看不见就容易怀疑它没干活。所以每次画像都会把过程摊出来给你看：

开工时先说清楚要翻多少：

```text
正在为 狐狸 生成人格棱镜：最多回溯 12 轮群历史（每轮 200 条），请稍候…
```

翻完、送进模型之前，报一遍实际数字：

```text
已翻 8 页群历史（约 1600 条消息，新入库 430 条），提取到 狐狸 的有效发言 300 条，正在生成人格棱镜…
```
...
本群语料状态：
  ...
  上次回溯：14 分钟前
  回溯断点：message_id=88123
```

「上次回溯」有时间就说明轮询成功过；「已往前翻过 N 页」是累计深度，**这个数字在涨就说明
回溯确实在往前推进**；「回溯断点」是下次继续往前挖的位置，等号前面是当前生效的游标字段。

如果诊断里出现「协议端的翻页没有继续前进」，说明两种游标字段都试过了但协议端就是不给翻页 ——
这种情况只能靠被动采集慢慢攒，或者换一个协议端。
...
结构化卡片包含：头像与昵称、一句人格标语、正负极性彩色标签、维度雷达图、
分节点评、原话证据（可一键关闭）、置信度、样本量与模型署名。
...
历史回溯靠 OneBot 的 `get_group_msg_history`，而这个接口的**翻页游标**是整个链路里最大的坑：
请求参数叫 `message_seq`，但它到底认哪个编号，各家实现并不一致。
...
最恶心的地方是**传错游标不会报错**：协议端会原地返回同一批最新消息。如果按「游标没变化」
来判断，就会误以为群历史已经挖到头了。

所以 v1.1.3 起改成了自适应策略，不需要你知道自己用的是哪种协议端：

1. 判断翻页是否前进，看的是「这一页有没有出现没见过的 `message_id`」，而不是比较游标数值。
2. 一旦发现原地打转，**自动换另一种游标字段重试**；哪种有效就记进数据库，之后直接沿用。
3. 两种都翻不动，就只记「协议端不支持翻页」并在采集诊断里说明，**不会**把这个群标成
   「历史已见底」而永久停止回溯。
4. 整数解析而不是 `str.isdigit()`，所以负数 `message_id` 也能正确当游标。

如果你已经确定自己协议端的行为，可以在配置里把「历史翻页游标」
（`collect.cursor_field`）从 `auto` 锁定为 `message_seq` 或 `message_id`，省掉一次试探。

**结论：go-cqhttp / NapCat / Lagrange / LLBot（幸运莉莉娅）都支持**，无需额外配置。
...
**配置**：把 43 项常用配置按 9 组铺开，带说明文字与取值范围；
两个敏感开关（同步机器人昵称 / 头像）在这里**只读展示**，只能在 AstrBot 原生配置页改，
避免误点一下就把机器人的全局资料给改了。

**提示词**：内置 10 套（棱镜 5 + 画像 5）可开关、可改写；也能新增自定义玩法
...
全部配置都能在 AstrBot 原生配置页改，其中 43 项也能在插件 WebUI 里改。下面按组说明。
...
| `cursor_field` | `auto` | 历史翻页游标字段。`auto` 自动试探并记住有效字段；也可锁定成 `message_seq` / `message_id` |
...
| 键 | 默认 | 说明 |
| --- | --- | --- |
| `legacy_commands` | 开 | 是否启用「画像」系列指令。要和上游插件共存就关掉 |
...
**要求给证据。** JSON 契约强制模型为每个结论附上原话引用；卡片上「原话证据」那一栏
就是拿来自证的 —— 有引用你能自己判断结论靠不靠谱，编的话一眼就看出来。
...
A：这是 v1.1.2 及更早版本的一个 bug —— 翻页游标字段在部分协议端上不生效，协议端不报错但
反复返回同一批最新消息，被误判成「群历史已见底」并永久停止回溯。**升级到 v1.1.3 即可**，
它会自动清零旧断点并自适应地重新试探游标字段。升级后在群里发一次画像，再发 `棱镜缓存`
看「已往前翻过 N 页」有没有涨起来。
...
但「画像系列」指令名和上游一样，同时启用会双响 —— 把 `compat.legacy_commands` 关掉即可。
...
| 4 | 历史翻页游标用 `isdigit()` 判断，NapCat 的负数 `message_id` 会被直接丢掉，静默停在第一页 | 整数解析，负数照样能翻；游标字段自动试探 `message_seq` / `message_id` 并记住有效的那个，翻不动只报诊断而不误判成「历史见底」 |
...
| 15 | 提示词没有输出格式约束、没有证据要求、也不防注入 | 强制输出契约（JSON / Markdown）+ 原话证据 + 置信度自评 + 反注入横幅，契约固定压在语料之后 |
...
- 453 个单元测试覆盖清洗、抽样、提示词、解析、存储、卡片、Markdown 渲染、采集诊断、翻页游标自适应、兼容层、WebUI 契约。
